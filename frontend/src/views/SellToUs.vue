<template>
  <main class="sell-to-us">
    <div class="main-text">
      ...
      <p>
        Fill out the form with a picture of the clothing and we'll come back to
        you with a fair appraisal from one of our experts!
      </p>
    </div>
    ...
    <v-form lazy-validation ref="form" v-model="valid" class="pl-4 pr-4">
      ...
      <v-text-field
        required
        :rules="emailRules.concat(notEmptyRule)"
        v-model="email"
        label="Email"
        outlined
        dense
      ...
      ></v-file-input>
      <v-text-field
        :rules="notEmptyRule.concat(onlyNumbers)"
        suffix="kr"
...
export default {
  ...
  data() {
    return {
      ...
      onlyNumbers: [v => /^[0-9]*$/.test(v) || "Only numbers is allowed"],
  ...
  methods: {
    sendRequest() {
      ...
      axios
        .post(`${process.env.VUE_APP_BACKEND}/sell_requests`, formData, {
          headers: {
        ...
        .then(function() {
          ...
          vue.dialog_text =
            "You're one step closer to getting paid! We will contact you by email regarding the sell request with an offer if we're interested. In the mean time, why don't you look at some other great clothes!";
        ...
        .catch(function(error) {
          ...
          vue.dialog_text =
            "Something happend while sending your sell request! Try again, or contact us via our contact page.";
          vue.dialog_buttonText = "Okay!";
