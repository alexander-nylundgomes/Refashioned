<template>
  <main class="filter">
      <header>
          ...
          <v-select outlined @change="filter()" dense class="pa-3 pt-0" hide-details label="Categories" :items="categories" item-text="name"></v-select>
          <v-select outlined @change="filter()" v-model="selectedSort" dense class="pa-3 pt-0" hide-details label="Sort by" item-text="text" item-value="id" :items="sorts"></v-select>
      </header>
...
export default {
    ...
    data(){
        return{
            ...
            sorts: [{text: 'Price: Low to High', id: 1}, {text: 'Price: High to Low', id: 2}],
