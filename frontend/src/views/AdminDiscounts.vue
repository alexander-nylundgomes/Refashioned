<template>
  <main class="discounts">
    <v-card class="ml-2 mr-2 mt-4 mb-6">
      ...
      <v-card-text>
        <v-form ref="form" lazy-validate>
          ...
          <v-text-field
            ...
            :rules="
              (type == 1 && percentLimitRules.concat(percentLimit)) ||
                percentLimitRules
            "
            dense
      ...
      <v-card-actions>
        <v-btn depressed block color="primary" @click="createDiscount()"
          >Create</v-btn
        >
      </v-card-actions>
    ...
    <section class="discounts" v-if="loaded">
      <Discount
        ...
        :key="discount.id"
        @deletedDiscount="removeDiscount($event)"
      />
    </section>
  </main>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      percentLimit: [
        v =>
          (parseInt(v) < 100 && parseInt(v) > 0) || "Must be between 0 and 100"
      ]
    };
  },
  ...
  methods: {
    async getDiscounts() {
      let vue = this;
      axios
        .get(`${process.env.VUE_APP_BACKEND}/discount_codes`)
        .then(function(resp) {
          console.log(resp);
    ...
    createDiscount() {
      if (this.$refs.form.validate()) {
        ...
        switch (this.type) {
          case 1:
            value_in_percent = this.discount_value;
            break;
          case 2:
            value_in_cash = this.discount_value;
            break;
          case 3:
            value_in_shipping = true;
            break;
        }

        let vue = this;
        axios
          .post(`${process.env.VUE_APP_BACKEND}/discount_codes`, {
            discount_code: {
          ...
          .catch(function(error) {
            vue.snackbarText =
              "Something went wrong! Look at console for more info.";
            console.log(error);
          });
    ...
    changeValueSuffix() {
      switch (this.type) {
        case 1:
          ...
          break;
        case 2:
          this.value_suffix = "kr";
          this.selectedShipping = false;
          break;
        case 3:
          this.value_suffix = "st";
          this.selectedShipping = true;
          break;
      }
    }
  },
