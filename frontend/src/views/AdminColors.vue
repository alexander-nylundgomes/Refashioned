  ...
  <main class="colors">
      <v-form lazy-validation ref="form" v-model="accepted">
        <v-card class="mt-5 mr-2 ml-2">
            ...
            <v-card-text>
                <v-text-field outlined dense v-model="name" label="Name" :rules="notEmptyRule"></v-text-field>
                <v-textarea outlined dense v-model="description" label="Description" :rules="notEmptyRule"></v-textarea>
                <v-color-picker :mode="'hexa'" v-model="newColor"></v-color-picker>
            </v-card-text>
            <v-card-actions>
                <v-btn color="primary" block depressed @click="create()" :disabled="!accepted">Create</v-btn>
            </v-card-actions>
        </v-card>
      </v-form>
    ...
    methods: {
        create(){
            if(this.$refs.form.validate()){
                let vue = this;
                axios.post(`${process.env.VUE_APP_BACKEND}/colors`, {
                    color: {
                        img_path: "",
                        name: vue.name,
