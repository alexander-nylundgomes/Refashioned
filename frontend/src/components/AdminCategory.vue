  ...
  <v-card class="mr-2 ml-2 mt-5">
    ...
    <v-dialog v-model="deleteAlert" width="500">
      <v-card>
        ...
        <v-card-text>
          <p>
            You are trying to delete a category. Multiple products will
            therefore have no category assigned to them. Please select a
            category to replace them with.
          </p>
          <v-select
            dense
            outlined
            label="Category"
            v-model="selectedReplaceCat"
          ...
          <v-btn
            color="primary"
            :disabled="selectedReplaceCat == null"
            class="mb-2"
            block
            depressed
            @click="destroy()"
...
export default {
  ...
  methods: {
    async update() {
      let vue = this;
      axios
        .patch(`${process.env.VUE_APP_BACKEND}/categories/${vue.category.id}`, {
          category: {
            name: vue.name,
    ...
    destroy() {
      ...
      axios
        .delete(
          `${process.env.VUE_APP_BACKEND}/categories/${vue.category.id}`,
          {
            params: {
