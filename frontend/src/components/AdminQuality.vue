<template>
  <v-card class="mr-2 ml-2 mt-5 quality-card">
      ...
      <v-card-text v-if="seeMore">
          <v-text-field dense outlined @input="updatePending = true" label="Grade" v-model="grade"></v-text-field>
          <v-text-field dense outlined @input="updatePending = true" label="Name" v-model="name"></v-text-field>
          <v-btn color="primary" class="mb-2" v-if="updatePending" block depressed @click="update()">Update</v-btn>
          <p><b>Created at:</b> {{ quality.created_at }}</p>
          <p><b>Updated at:</b> {{ quality.updated_at }}</p>
          <v-divider class="mb-5 mt-5"></v-divider>
          <v-checkbox label="Do you want to delete this quality?" v-model="accessDelete"></v-checkbox>
          <v-btn color="primary" class="mb-2" :disabled="!accessDelete" block depressed @click="destroy()">Delete</v-btn>
      </v-card-text>
      <v-card-actions >
...
export default {
    ...
    methods: {
        async update(){
            let vue = this;
            axios.patch(`${process.env.VUE_APP_BACKEND}/qualities/${vue.quality.id}`,{
                quality: {
                    name: vue.name,
                    grade: vue.grade,
                }
