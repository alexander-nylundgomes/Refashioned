<template>
  <v-card class="mt-5 mr-2 ml-2">
      ...
      <v-card-text v-if="seeMore">
          <v-text-field dense outlined v-model="value" @input="updatePending = true" label="Value"></v-text-field>
          <v-btn color="primary" block depressed class="mb-4" v-if="updatePending" @click="update()">Update</v-btn>
          <p><b>Created at:</b> {{misc.created_at}}</p>
          <p><b>Updated at:</b> {{misc.updated_at}}</p>
      </v-card-text>
      <v-card-actions>
          <v-btn depressed :outlined="seeMore" color="primary" @click="seeMore = !seeMore" block>{{ !seeMore && 'See more' || 'See less' }}</v-btn>
      </v-card-actions>
...
export default {
    ...
    methods: {
        async update(){
            ...
            axios.patch(`${process.env.VUE_APP_BACKEND}/miscs/${vue.misc.id}`, {
                misc: {
