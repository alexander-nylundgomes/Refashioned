<template>
  <v-card class="mr-2 ml-2 mt-5">
    ...
    <v-card-text v-if="seeMore">
      ...
      <v-color-picker
        :mode="'hexa'"
        @update:color="updatePending = true"
    ...
    <v-dialog v-model="deleteAlert" width="500">
      <v-card>
        ...
        <v-card-text>
          <p>
            You are trying to delete a color. Multiple products will therefore
            have no color assigned to them. Please select a color to replace
            them with.
          </p>
          <v-select
            dense
            outlined
            label="Color"
            v-model="selectedReplaceColor"
            :items="$store.getters.colors"
            item-text="name"
            item-value="id"
          ></v-select>
          <v-btn
            color="primary"
            :disabled="selectedReplaceColor == null"
            class="mb-2"
            block
            depressed
            @click="destroy()"
...
export default {
  ...
  methods: {
    async update() {
      let vue = this;
      axios
        .patch(`${process.env.VUE_APP_BACKEND}/colors/${vue.color.id}`, {
          color: {
    ...
    destroy() {
      ...
      let vue = this;
      axios
        .delete(`${process.env.VUE_APP_BACKEND}/colors/${vue.color.id}`, {
          params: {
