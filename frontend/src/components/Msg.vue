<template>
  <v-card class="ma-3" :class="{ readMessage: msg.read }">
    ...
    <v-card-subtitle class="message-text"
      >{{ msg.subject }} - {{ msg.firstname }}
      {{ msg.lastname }}</v-card-subtitle
    >
...
export default {
  ...
  methods: {
    updateReadMessage() {
      ...
      axios
        .patch(`${process.env.VUE_APP_BACKEND}/msgs/${vue.msg.id}`, {
          msg: {
            read: !vue.msg.read
          }
        })
