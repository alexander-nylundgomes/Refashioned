    ...
    <v-lazy>
        <v-card class="mr-2 ml-2 mb-6 mt-2">
            ...
            <v-card-text v-if="seeMore">
                ...
                <v-select dense outlined hide-details @change="pendingUpdates = true" v-model="currentStatus" class="mt-4 mb-2" :items="statuses"></v-select>
                <v-btn color="primary" depressed block @click="update()" v-if="pendingUpdates">Update</v-btn>
            </v-card-text>
            <v-card-actions>
                <v-btn depressed color="primary" v-if="!seeMore" @click="seeMore = true" block>See more</v-btn>
                <v-btn depressed color="primary" v-if="seeMore" @click="seeMore = false" outlined block>See less</v-btn>
            </v-card-actions>
...
export default {
    ...
    methods: {
        async update(){
            let vue = this;
            axios.patch(`${process.env.VUE_APP_BACKEND}/sell_requests/${vue.sr.id}`, {
                sell_request: {
